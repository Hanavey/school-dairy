...
class AdminOneStudentAPI(Resource):
    @staticmethod
    def get(student_id):
        username = check_admin_authorization(f"/api/admin/student/{student_id}", method="GET")
    ...
    def patch(student_id):
        username = check_admin_authorization(f"/api/admin/student/{student_id}", method="PATCH")
        db_sess = db_session.create_session()
        ...
        try:
            ...
            args = one_student_patch.parse_args()

            # Обновляем только переданные поля для объекта User
            user = student.user
            if args['username'] is not None:
                user.username = args['username']
            if args['password'] is not None:
                user.set_password(args['password'])  # Предполагается, что пароль уже хешируется где-то в модели
            ...
            # Обновляем только переданные поля для объекта Student
            ...
            db_sess.commit()
            logging.info(f"PATCH /api/admin/student/{student_id} - Updated by user {username}")

            # Возвращаем обновленные данные студента (опционально)
            return {
    ...
    def post():
        ...
        try:
            ...
            if db_sess.query(User).filter(User.username == args['username']).first():
                logging.error(f"POST /api/admin/students - Username {args['username']} already exists")
            ...
            if db_sess.query(User).filter(User.email == args['email']).first():
                logging.error(f"POST /api/admin/students - Email {args['email']} already exists")
                abort(400, message=f"Email {args['email']} уже зарегистрирован.")
            ...
            # Преобразуем birth_date в объект datetime.date
            birth_date = parse_date(args['birth_date'])

            class_exists = db_sess.query(Class).filter(Class.class_id == args['class_id']).first()
            if not class_exists:
                logging.error(f"POST /api/admin/students - Class {args['class_id']} not found")
                abort(400, message=f"Класс с ID {args['class_id']} не найден.")

            # Создаем нового студента
            new_student = Student(
                user_id=new_user.user_id,
                student_id=(db_sess.query(func.max(Student.student_id)).scalar() or 0) + 1,
            ...
            # Фиксируем изменения в базе данных
            db_sess.commit()
            logging.info(f"POST /api/admin/students - Created student with user_id {new_user.user_id} by {username}")
            classes = {'class_id': new_student.class_id, 'class_name':
                db_sess.query(Class).filter(Class.class_id == new_student.class_id).first().class_name}

            # Возвращаем данные о созданном студенте
            return {
    ...
    def delete(student_id):
        username = check_admin_authorization(f"/api/admin/student/{student_id}", method="DELETE")
        db_sess = db_session.create_session()
